...
func combineLastCharacters(_ strArray:[String]) -> String{
    var strlastLetters = ""
    for str in strArray{
        strlastLetters.append(str.characters.last!)
    }
    return strlastLetters
}
...
func isDigits(price:String) -> Bool{
    price.decimalDigitCharacterSet()
...
func removeFourStr(strArrays:[String])-> [String]{
    var removedirtyWordsArray = [String]()
    for str in strArrays{
        if(str.characters.count != 4 ){
            removedirtyWordsArray.append(str)
        }
    }
    return removedirtyWordsArray
}

removeFourStr(strArrays: dirtyWordsArray)
...
func filterByDirector(director:String) ->[String]{
    var movieTitle = [String]()
    for (key, value) in movies{
        if(value.contains(director)){
            movieTitle.append(key)
        }
    }
    return movieTitle
}
...
class MovieArchive {
    ...
    init(director:String) {
                            // label : arg name
        movie = filterByDirector(director: director)
    }
}

MovieArchive.init(director: "Christopher Nolan").movie
